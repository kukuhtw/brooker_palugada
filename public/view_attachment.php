...
$sqlInv = "
  SELECT id, owner, phone, email, rawdata, description
    FROM data_inventory
   WHERE id = :id
";
$stmtInv = $db->getConnection()->prepare($sqlInv);
...
<body class="d-flex">
 ...
 <!-- Judul memakai owner dan ID -->
    <h1 class="mb-3">
      Lampiran untuk Data #<?= htmlspecialchars($inv['id']) ?>
      (<?= htmlspecialchars($inv['owner']) ?>)
    </h1>

    <!-- 2) Tampilkan ID, Owner, Phone, Email, Description -->
    <ul class="list-unstyled mb-4">
      <li><strong>ID:</strong> <?= htmlspecialchars($inv['id']) ?></li>
