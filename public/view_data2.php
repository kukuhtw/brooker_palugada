...
// Hitung total
$countSql = "SELECT COUNT(*) FROM data_inventory" . ($ownerFilter ? " WHERE owner LIKE :owner" : "");
$countStmt = $db->getConnection()->prepare($countSql);
if ($ownerFilter) {
    $countStmt->execute([':owner' => "%{$ownerFilter}%"]);
} else {
    $countStmt->execute();
}
...
// Ambil data
$sql = "SELECT id, owner, phone, email, rawdata, description, metadata_pinecone, ispinecone, regdate
        FROM data_inventory" 
     . ($ownerFilter ? " WHERE owner LIKE :owner" : "")
     . " ORDER BY lastupdatedate DESC
        LIMIT :limit OFFSET :offset";

$stmt = $db->getConnection()->prepare($sql);
if ($ownerFilter) {
    $stmt->bindValue(':owner', "%{$ownerFilter}%", PDO::PARAM_STR);
}
...
<body class="d-flex">
  ...
  <div id="content" class="flex-grow-1 p-4">
    <h1 class="mb-4">Daftar Data Inventory</h1>
    ...
    <form method="get" class="row g-2 mb-4">
      <div class="col-auto">
        <input type="text" name="owner" class="form-control"
               placeholder="Cari Owner Name" value="<?= htmlspecialchars($ownerFilter) ?>">
      </div>
      <div class="col-auto">
      ...
      <?php if($ownerFilter): ?>
      <div class="col-auto">
        <a href="view_data.php" class="btn btn-secondary">Reset</a>
      </div>
      <?php endif; ?>
    </form>
    ...
    <div class="table-responsive">
        ...
        <tbody>
          ...
          <?php else: foreach($rows as $row): ?>
            <tr>
              ...
              <td>
                <?= nl2br(htmlspecialchars(substr($row['rawdata'],0,100))) ?>
                <?= strlen($row['rawdata'])>100?'…':'' ?>
              </td>
              <td>
                <?php if (trim($row['description']) === ''): ?>
                  <a href="proses_info_desc.php?data_id=<?= $row['id'] ?>&ownerFilter=<?= urlencode($ownerFilter) ?>&page=<?= $page ?>"
                ...
                <?php if (trim($row['description']) !== '' && trim($row['metadata_pinecone'] ?? '') === ''): ?>
                  <p>&nbsp;</p>
                  <a href="proses_metadata.php?data_id=<?= $row['id'] ?>&ownerFilter=<?= urlencode($ownerFilter) ?>&page=<?= $page ?>"
                     class="btn btn-sm btn-warning">Proses Metadata Pinecone</a>
                <?php endif; ?>
              </td>
              <td>
                <?= nl2br(htmlspecialchars(substr($row['metadata_pinecone'],0,100))) ?>
                <?= strlen($row['metadata_pinecone'])>100?'…':'' ?>
      ...
      <?php 
// Ambil dan trim metadata
  $meta = trim($row['metadata_pinecone'] ?? '');

  // Cek apakah metadata bukan string kosong, lalu decode & periksa error
  $isValidJson = false;
  if ($meta !== '') {
      json_decode($meta);
 ...
 <!-- Tombol Add Attachment -->
  ...
  <!-- Tombol View Attachments -->
  <a href="view_attachment.php?data_id=<?= $row['id'] ?>
           &owner=<?= urlencode($ownerFilter) ?>
           &page=<?= $page ?>"
    ...
    View Attachments
            ...
            <!-- Tombol Hapus -->
           <form method="post" action="delete_data_inventory.php" class="d-inline"
    ...
    <nav><ul class="pagination">
      <li class="page-item<?= $page<=1?' disabled':'' ?>">
        <a class="page-link" href="?<?= http_build_query(['owner'=>$ownerFilter,'page'=>$page-1]) ?>">&laquo;</a>
      </li>
      <?php for($i=1;$i<=" . " .$totalPages;$i++): ?>
      <li class="page-item<?= $i===$page?' active':'' ?>">
        <a class="page-link" href="?<?= http_build_query(['owner'=>$ownerFilter,'page'=>$i]) ?>"><?= $i ?></a>
      </li>
      <?php endfor; ?>
      <li class="page-item<?= $page>=$totalPages?' disabled':'' ?>">
        <a class="page-link" href="?<?= http_build_query(['owner'=>$ownerFilter,'page'=>$page+1]) ?>">&raquo;</a>
      </li>
    </ul></nav>
    <?php endif; ?>
  </div>
